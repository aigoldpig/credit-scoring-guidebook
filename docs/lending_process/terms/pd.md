...
**맥락에 따라 가리키는 대상이 다릅니다.**
이 페이지에서는 "PD"라는 단어가 실무에서 어떤 층위로 쓰이는지 정리합니다.
...
!!! note "가이드북에서의 위치"
    ...
    log-odds → sigmoid → p̂ 변환 과정을 다루고 있습니다.

---

## 2. 등급별 PD — 관측 부도율

p̂을 실무에서 바로 쓰지는 않습니다. 두 단계의 변환을 거칩니다.

### 평점(Score) 변환

p̂을 PDO(Points to Double the Odds)와 Anchor Score 개념으로
사람이 읽기 편한 **정수 점수**로 변환합니다.

$$
\text{Score} = A - B \times \ln(\text{Odds})
$$

여기서 \(A\)는 Anchor Score, \(B = \frac{\text{PDO}}{\ln 2}\) 입니다.

!!! note "가이드북에서의 위치"
    [스코어카드 Part 5 — 스코어카드 변환 & 등급화](../../scorecard/part5_scorecard/scorecard-and-rating.md)에서
    PDO 변환과 등급화 과정을 다루고 있습니다.

### 등급화

평점 분포를 구간으로 나누어 1~10등급(또는 기관별 등급 체계)으로 매핑합니다.
Beta 분포 시뮬레이션 등을 활용하여 등급 경계를 설정하며,
각 등급에 속한 차주들의 **관측 부도율**이 곧 **등급별 PD**가 됩니다.
...
이후 운영 시 **back rating**을 통해 산출한 해당 집단의 부도율입니다.

### 한계

등급별 PD는 **관측 시점의 경기 상황이 그대로 묻어 있습니다.**

- 호황기에 관측하면 PD가 낮게 나옴
- 불황기에 관측하면 PD가 높게 나옴

이 한계를 보정하는 것이 아래의 TtC PD와 PiT PD입니다.
...
Basel 내부등급법(IRB)은 이렇게 요구합니다:

> "경기 한 사이클을 평균낸 **장기 부도율**을 PD로 써라."

규제자본의 목적이 **어떤 경기 국면이든 버틸 수 있는 완충 장치**이기 때문입니다.
...
**경기순응성(procyclicality)** 문제가 생깁니다. TtC PD는 이를 완화합니다.

### CT (Central Tendency)

TtC PD의 기준점이 되는 **장기 평균 부도율**을 CT라고 합니다.
...
!!! info "기업 모형에서 CT가 특히 중요한 이유"
    개인(Retail)과 달리 기업(Corporate) 데이터는 두 가지 특수성이 있습니다.

    1. **부도가 드물다** (Low Default Portfolio) — 1~2년 치로는 부도율 추정이 어려움
    2. **경기를 많이 탄다** — 업종에 따라 호황 0% ↔ 불황 10% 수준의 변동

    호황기 데이터만으로 모형을 만들면 PD를 과소평가하는 위험이 발생합니다.
    리테일은 부도 건수가 충분하여 CT를 명시적으로 세팅하지 않는 경우도 있지만,
    Basel IRB가 TtC를 요구하는 것은 기업·리테일 공통입니다.

### 레벨링 (Calibration)

모형 개발은 최근 3~5년 데이터로 하되, **PD 수준은 CT에 맞추는** 과정입니다.

> **변별력(Ranking)은 최근 데이터로, 레벨(Level)은 장기 데이터(CT)로.**

로지스틱 회귀의 경우 **상수항(intercept)만 조정**하여 평균 PD가 CT에 맞도록 합니다.
...
변수 계수(\(\beta_1, \beta_2, \ldots\))는 건드리지 않으므로 **서열은 그대로, 수준만 이동**합니다.

!!! tip "ML 모형의 레벨링"
    GBM 같은 ML 모형은 로지스틱 회귀처럼 상수항을 조정하는 구조가 아닙니다.
    대신 실무에서는 **등급 매핑 단계에서 보정**하는 방식이 가장 흔합니다.

    - ML 모형의 p̂으로 **서열**을 만들고
    - 등급별 PD를 붙일 때 **CT 기준 테이블**을 사용

    즉 모형 자체는 안 건드리고, 등급별 PD 숫자만 CT로 교체합니다.
    모형이 LR이든 GBM이든 **서열만 잘 매기면 되고, 레벨은 등급 테이블에서 잡는다**는
    구조는 동일합니다.

레벨링 후 각 등급의 PD가 CT 기준으로 재산출되며, 이것이 **TtC PD**입니다.
...
IFRS 9 (2018년~)는 TtC PD와 **반대 방향**의 요구를 합니다:

> "지금 경기 상황을 반영한 PD를 써라."

배경은 2008년 금융위기입니다.
이전 회계기준(IAS 39)은 **발생 손실(Incurred Loss)** — 부도가 실제로 터져야 충당금을 잡는 구조였습니다.
결과적으로 손실이 한꺼번에 인식되어 재무제표가 급락했고, 시장 패닉을 가속했습니다.

IFRS 9는 이를 뒤집어서 **기대신용손실(Expected Credit Loss, ECL)** —
아직 터지지 않았어도 터질 것 같으면 미리 충당금을 잡도록 했습니다.

TtC PD는 경기 중립적이므로 경기가 나빠져도 PD가 안 움직입니다.
하지만 IFRS 9는 "경기가 나빠지면 충당금을 더 쌓아라"를 원하므로, **현재 경기를 반영하는 PiT PD**가 필요합니다.

### 산출 방식

TtC PD를 출발점으로, **거시경제 변수를 반영하여 조정**합니다.
...
PiT PD가 산출되면, 대출 건별로 Stage를 분류합니다.
