...
SHAP과 fANOVA는 모두 "변수별 기여"를 구하지만, **교호작용(interaction)을 처리하는 방식**이 근본적으로 다르다.
...
교호작용의 기여분을 \(X_1\)과 \(X_2\)에게 **반씩 나눠준다.**
...
소득은 같은데 SHAP 값이 다르다. 이유는 소득×부채 교호작용의 기여분이 각 샘플에서 **다른 크기로** 소득의 SHAP에 흡수되기 때문이다. 차주 A에서는 "고소득+저부채" 시너지가 크게 작동하여 소득의 몫이 커지고, 차주 B에서는 시너지가 약해서 소득의 몫이 줄어든다.
...
교호작용을 분배하지 않고, **별도의 항으로 분리**한다. 직교 제약 조건([2절](functional-anova.md), 식 2~5)에 의해 각 항이 **유일하게** 결정된다:
...
SHAP과 fANOVA는 접근 방식이 다르지만, **분포에 의존한다**는 근본적 한계를 공유한다.
...
fANOVA는 global 해석 도구이고, 개별 샘플 설명은 SHAP의 영역이라고 흔히 생각한다. 하지만 교호작용 차수를 제한한 모형에서는 fANOVA가 오히려 **더 정직한 개별 설명**을 제공할 수 있다.

### 2-way까지 제한된 모형 — 설명 가능

GA\(^2\)M(EBM)이나 depth-2 트리처럼 교호작용을 2-way로 제한하면, 개별 고객의 예측을 component별로 제시할 수 있다:
...
교호작용을 변수에 나눠줄 필요 없이, **별도의 설명 항목으로 그대로 제시**하면 된다. \(f_{12}\)는 2차원 heatmap으로 시각화도 가능하다. 이 방식은 SHAP보다 **더 정직한 설명**이다 — 교호작용이 숨겨지지 않으니까.
...
### 3-way 이상 — 해석이 급격히 어려워진다

그런데 depth가 깊어져서 \(X_1, X_2, X_3\)가 혼재된 3-way interaction이 나타나면 상황이 달라진다.
...
3-way만 허용해도 1,140개의 interaction 항이 가능하다. 이 중 유의미한 것만 선별하더라도, **하나하나의 해석 자체가 어렵다** (3차원을 직관적으로 이해하기 힘드니까). 설명 항 수의 문제와 개별 항의 이해 가능성 문제가 동시에 발생한다.

**"설명 가능한 depth 제한"이라는 것**: depth가 5 이상이더라도, 변수 조합을 강제로 \((X_1, X_2)\) 같은 2-way pair로만 구성하면 각 pair는 2D heatmap으로 표현할 수 있다. 핵심은 트리의 depth가 아니라 **하나의 interaction 항에 관여하는 변수 수**다. EBM이 2-way로 제한하는 것은 바로 이 이유 — 2차원까지는 그림으로 보여줄 수 있고, 3차원부터는 안 되기 때문이다.
