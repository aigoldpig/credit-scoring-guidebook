...
[2절](functional-anova.md)에서 fANOVA의 직교 제약(Zero Means, Orthogonality, Integrate-to-Zero)이 분해를 유일하게 결정한다는 것을 확인했다. 문제는, 트리 기반 모형(XGBoost, EBM 등)이 학습 과정에서 이 제약을 **전혀 고려하지 않는다**는 것이다. 트리는 단지 예측 오차를 줄이는 방향으로 분할할 뿐이고, "이 효과가 main에 담겨야 하는가, interaction에 담겨야 하는가"를 구분하지 않는다.
...
"main effect를 먼저 학습하고, 나머지를 교호작용으로 돌리면 누출이 없지 않을까?" 합리적인 기대이지만, 그럼에도 **누출이 발생**하는 이유가 있다:
