...
1-Depth GBM의 가장 큰 장점은 **각 변수의 효과를 시각화**할 수 있다는 것이다.
...
이 shape function은 해당 변수가 부도 확률에 미치는 효과를 보여주며, **심사 담당자와 규제 기관 모두에게 직관적으로 설명 가능**하다.
...
!!! tip "Depth = 2도 좋은 선택지: PiML의 XGB2"
    Depth = 2이면 2-way 교호작용까지 포착하면서도, purification으로 주효과와 교호작용을 깔끔하게 분리할 수 있다. 1-Depth보다 성능이 의미 있게 올라가는 경우가 많다.

    [PiML](https://github.com/SelfExplainML/PiML-Toolbox) 라이브러리는 **XGB2** (Depth-2 XGBoost + purification)를 별도의 해석 가능 모형 유형으로 지원한다. XGB2의 작동 원리는 다음과 같다:

    1. **학습**: 일반 XGBoost와 동일하되, `max_depth=2`로 제한하여 각 트리가 최대 2번만 분할
    2. **재구성**: 학습된 모든 트리의 leaf node 정보를 추출하여, Functional ANOVA 관점에서 GA\(^2\)M (GAM with pairwise interactions) 형태로 재구성
    3. **Purification**: 교호작용 항에 섞여 있는 주효과 성분을 분리하여, 각 \(f_j\)는 순수 주효과만, 각 \(f_{jk}\)는 순수 교호작용만 포함하도록 정제

    결과적으로 XGB2 모형은 main effect plot(변수별 효과 곡선)과 interaction plot(2-way 교호작용 히트맵)을 모두 깔끔하게 제공하며, local 해석도 "이 예측값은 주효과 A + 주효과 B + 교호작용 A×B로 구성된다"는 형태로 분해된다. 단조 제약(`monotone_constraints`)도 지원한다.

!!! abstract "이론 심화: 부록 A — SHAP과 Functional ANOVA"
    이 페이지에서 다룬 GAM 분해, Purification, SHAP과의 관계 등은 **Functional ANOVA**라는 수학적 프레임워크 위에 서 있다. 분해의 비유일성(non-identifiability) 문제가 왜 발생하는지, fANOVA가 어떤 제약으로 유일한 분해를 보장하는지, Purification 알고리즘이 구체적으로 어떻게 작동하는지에 대한 세부 이론은 [부록 A: SHAP과 Functional ANOVA](../appendix/shap-fanova/index.md)에서 다룬다.
