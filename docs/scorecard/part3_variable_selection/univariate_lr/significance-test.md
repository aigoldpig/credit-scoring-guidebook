...
!!! example "CB사 실무 사례: 단변량 로지스틱 회귀 유의성 기준"
    국내 CB사(NICE, KCB)의 CSS 개발 실무에서는 단변량 로지스틱 회귀의 유의성 기준을 다음과 같이 적용한다:

    - **변수 전체 LRT p-value < 0.05**를 1차 스크리닝 기준으로 사용
    - Bin별 Wald p-value는 **0.05 ~ 0.10 사이**를 허용 범위로 두되, 단조성 유지 여부를 우선 고려
    - 금감원 모형검증 가이드라인에서는 개별 변수의 통계적 유의성과 함께 **업무적 설명력**을 종합 판단할 것을 권고
