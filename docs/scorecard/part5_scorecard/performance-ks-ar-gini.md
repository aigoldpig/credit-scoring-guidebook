...
!!! example "실무 참고 — 신용평가모형 성능 벤치마크"
    개인 CSS(Credit Scoring System)는 CB 신용거래정보(연체·대출·카드 이용 등)를 주력 변수로 활용하며, 일반적으로 **KS 50~65, AR 0.65~0.80** 수준의 변별력 대역이 보고되고 있다. 기업 CSS의 경우 재무정보 + CB 거래정보 결합 모형에서 **KS 35~50** 수준이 일반적이다.

    비금융 대안정보(통신비 납부, 공공요금 등)를 추가한 Thin-file 전용 모형은 기존 대비 변별력 향상 효과가 있는 것으로 알려져 있다.
