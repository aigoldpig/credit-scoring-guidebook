...
!!! example "CB사 참고 — 국내 모형 운영 주기"
    **NICE평가정보**와 **KCB**의 주요 신용평점 모형은 통상 **2~3년 주기**로 정기 재개발을 수행한다. 그 사이 분기별 모니터링에서 성능 열화가 감지되면 리캘리브레이션으로 대응하고, 열화가 심각하면 정기 주기를 앞당겨 조기 재개발에 착수한다.

    금융감독원의 **신용평가모형 검증 가이드라인**에서도 최소 연 1회 이상 모형 성능을 검증하고, PSI 등 안정성 지표를 분기별로 모니터링하도록 권고하고 있다.
...
!!! info "규제 요건 상세"
    Basel IRB 검증 요건, 금감원 가이드라인, SR 11-7 등 규제 프레임워크의 상세 내용은 [규제 프레임워크](regulatory-framework.md)에서 다룬다.
