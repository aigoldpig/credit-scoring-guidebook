...
!!! example "실무 참고 — OOT 검증 관행"
    신용평가모형 개발 시 최소 1개 이상의 OOT 기간을 설정하여 검증하는 것이 업계 표준이다. 개인 CSS의 경우 개발 기간 2~3년, OOT 기간 1년이 일반적이며, 기업 CSS는 불량 건수 확보를 위해 개발 기간을 더 길게 설정하는 경우가 많다.

    Basel IRB 프레임워크에서도 OOT 검증을 요구하고 있으며, OOT 없이 배포된 모형은 검증 미흡으로 판정될 수 있다.
