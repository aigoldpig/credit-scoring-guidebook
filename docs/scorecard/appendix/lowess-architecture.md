...
!!! note "프로빗 vs 로짓"
    본 교재에서 다룬 소매 CSS는 **로짓(logit)** 모형을 사용한다. RiskCalc는 **프로빗(probit)** 모형을 채택한다. 두 모형의 차이는 링크 함수(\(\Phi^{-1}\) vs \(\text{logit}\))뿐이며, 실무적 성능 차이는 미미하다. 프로빗은 정규분포 가정과의 정합성, 로짓은 Odds Ratio 해석의 편의성이 장점이다.
