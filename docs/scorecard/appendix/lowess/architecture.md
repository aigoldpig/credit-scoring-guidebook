...
!!! note "왜 프로빗인가?"
    본 가이드북에서 다룬 소매 CSS는 **로짓(logit)**, RiskCalc는 **프로빗(probit)**을 사용한다. 실무적 성능 차이는 거의 없고 링크 함수(\(\Phi^{-1}\) vs \(\text{logit}\))만 다르다.
    ...
    반면 소매 CSS는 **통계적 판별/스코어링** 전통에서 출발하여, odds ratio 해석이 직관적인 로짓이 표준이 되었다.
