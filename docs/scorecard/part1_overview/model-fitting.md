...
!!! note "현실적 선택"
    "로지스틱 회귀냐 ML이냐"는 양자택일이 아니다. 규제모형은 전통 스코어카드로, 전략 모형은 ML로 개발하거나, ML로 변수 탐색 → 로지스틱 회귀로 최종 적합하는 하이브리드 접근이 실무에서 가장 흔하다. 본 교육자료의 Part 1~6은 **전통 스코어카드 파이프라인**을 중심으로 다루며, ML 기반 신용평가의 상세 내용(트리 모형, Feature Engineering, 해석가능성, 검증 등)은 [머신러닝 섹션](../../ml/index.md)에서 별도로 다룬다.
